package com.xiaoshabao.framework.wechat.apiJW.wxshop;
...
import com.xiaoshabao.framework.wechat.apiJW.report.interfacesummary.model.InterfaceSummary;
import com.xiaoshabao.framework.wechat.apiJW.wxshop.model.Business;
import com.xiaoshabao.framework.wechat.apiJW.wxshop.model.BusinessReq;
import com.xiaoshabao.framework.wechat.apiJW.wxshop.model.PoiId;
import com.xiaoshabao.framework.wechat.apiJW.wxshop.model.ShopRtnInfo;
...
public class JwShopAPI {
	...
	public static ShopRtnInfo doAddshop(String newAccessToken, Business business) {
		if (newAccessToken != null) {
			...
			JSONObject result = WxstoreUtils.httpRequest(requestUrl, "POST",
					jsonString);
			ShopRtnInfo shopRtnInfo = JSONObject.toJavaObject(result,
					ShopRtnInfo.class);
			return shopRtnInfo;
		}
		return null;
	...
	public static ShopRtnInfo delShop(String newAccessToken, PoiId poiId) {
		if (newAccessToken != null) {
			String requestUrl = del_shop_url.replace("ACCESS_TOKEN",
					newAccessToken);
			String jsonString = JSON.toJSONString(poiId);
			JSONObject result = WxstoreUtils.httpRequest(requestUrl, "POST",
					jsonString);
			ShopRtnInfo shopRtnInfo = JSONObject.toJavaObject(result,
					ShopRtnInfo.class);
	...
	public static BaseInfo getshop(String newAccessToken, PoiId poiId) {
		if (newAccessToken != null) {
			...
			baseInfo = JSONObject.toJavaObject(info, BaseInfo.class);
			return baseInfo;
		}
		return null;
	}
	...
	public static List<BaseInfo> getshops(String newAccessToken,
		...
		if (newAccessToken != null) {
			String requestUrl = search_shop_url.replace("ACCESS_TOKEN",
					newAccessToken);
			String jsonString = JSON.toJSONString(businessReq);
			JSONObject result = WxstoreUtils.httpRequest(requestUrl, "POST",
					jsonString);
			// 正常返回
			List<BaseInfo> baseInfos = null;
			JSONArray info = result.getJSONArray("business_list");
			baseInfos=JSONArray.parseArray(info.toJSONString(), BaseInfo.class);
			return baseInfos;
		}
		return null;
	}
	...
	public static String uploadImg(String newAccessToken, String filePath,
		...
		if (newAccessToken != null) {
			...
			try {
				...
				JSONObject result = WxstoreUtils.httpRequest2(requestUrl,
						"POST", fileByte);
