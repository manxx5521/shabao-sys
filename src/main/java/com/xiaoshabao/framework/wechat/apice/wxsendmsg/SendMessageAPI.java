package com.xiaoshabao.framework.wechat.apice.wxsendmsg;
...
import com.xiaoshabao.framework.wechat.apice.wxsendmsg.model.SendMessageReport;
import com.xiaoshabao.framework.wechat.apice.wxsendmsg.model.SendMessageResponse;
...
import com.xiaoshabao.framework.wechat.apice.wxsendmsg.model.WxMedia;
import com.xiaoshabao.framework.wechat.apice.wxsendmsg.model.WxMediaResponse;

public class SendMessageAPI {
	private static Logger logger = LoggerFactory.getLogger(SendMessageAPI.class);
	// 消息预览URL
	...
	public static SendMessageResponse sendMessageToGroupOrAllWithArticles(String accesstoken, boolean is_to_all, Group group, List<WxArticle> wxArticles) throws WexinReqException {
		...
		if (accesstoken != null) {
			...
			try {
				...
				JSONObject result = WxstoreUtils.httpRequest(requestUrl, "POST", obj.toString());
				//System.out.println("微信返回的结果：" + result.toString());
				response =  JSONObject.toJavaObject(result, SendMessageResponse.class);
			} catch (Exception e) {
...
//		WxMediaResponse response = uploadMediaFile(accesstoken, article.getFilePath(), article.getFileName(), "image");
		String path=article.getFilePath()+article.getFileName();
		WxUpload response = MediaAPI.uploadTempMedia(accesstoken,MediaType.IMAGE, path);
		if (response != null) {
			return response.getMedia_id();
		}
