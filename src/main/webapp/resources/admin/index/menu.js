...
var getMenuL1 = function(obj) {
	var li = $('<li></li>').attr('menu_id', obj.menu_id);
	// 设置a
	var a = $('<a></a>');
	// 如果是菜单按钮
	if (obj.ismenu == 1) {
		a.addClass('J_menuItem');
		a.attr('href', webRoot+obj.url);
	} else {
		a.attr('href', '#');
	}
...
var getMenuL2 = function(obj) {
	//定义获取li的字符串
	var s_pli='li[menu_id='+obj.parent_menu_id+']';
	var li_pid=$(s_pli);
	//如果没有父级标签返回
	if(li_pid.length<1)
		return false;
	...
	a.text(obj.menu_title);//设置菜单名
	var li = $('<li></li>').attr('menu_id', webRoot+obj.menu_id);//二级标题的li
	li.append(a);
...
var getMenuL3 = function(obj) {
	...
	a.text(obj.menu_title);//设置菜单名
	var li = $('<li></li>').attr('menu_id', obj.menu_id);//三级标题的li
	li.append(a);
