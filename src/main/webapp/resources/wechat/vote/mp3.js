createbgmusic = function(state) {
	var playbox = (function(state) {
		var _playbox = function() {
			...
			that.autoPlayFix = {
				...
				evtName : ("ontouchstart" in window) ? "touchstart"
						: "mouseover"
			}
		};
		_playbox.prototype = {
			...
			play : function() {
				...
				this.on = !this.on;
				if (true == this.on) {
					this.player.src = $("#audio").attr('dsrc');
					this.player.play();
				} else {
					this.player.pause();
			...
			autoPlayEvt : function(important) {
				if (important || this.autoPlayFix.on) {
					document.body.addEventListener(this.autoPlayFix.evtName,
							this, false);
				} else {
	...
	playbox.play_fn = function() {
		this.box.className = this.on ? "btn_music musicplay"
				: "btn_music musicstop";
	};
